## Hi there 👋

### About Me

🎓 I am a graduate student majoring in Software Engineering. My research focuses on combining artificial intelligence and neuroscience, especially in exploring advanced brain-computer interface (BCI) technologies and intelligent neural decoding algorithms.

### Research & Technical Interests

- 🧠 AI and neuroscience integration
- 🧩 Brain-computer interface (BCI) technology
- 🧑‍💻 Modern graphics programming and game development
- 💻 Programming languages: C++, Python

### Featured Project

- 🛠️ [**ReduxGL**](https://github.com/VerdantE1/A-Modern-C-20-OpenGL-Rendering-Engine-with-Object-Oriented-Library):  
  A modern C++20-based OpenGL rendering engine framework designed with object-oriented principles and advanced graphics techniques.  
  - OpenGL Core Profile 3.3+  
  - Flexible shader and resource management  
  - Dynamic scene rendering  
  - Demonstrates best practices in modern graphics programming
- 🛠️ [**PyMagicBox**](https://github.com/VerdantE1/PyMagicBox):
  - Tools generating bat for windows used python to boost productivity.
### Hobbies & Topics I Enjoy

- 🎮 Interested in all aspects of graphics programming and game development, from engine architecture to gameplay systems and visual effects.
- 🕹️ Enjoy a wide variety of games, especially strategy games (Paradox Interactive titles, The Elder Scrolls series, Bethesda games, etc.).
- 🌐 Also active in online games, such as League of Legends (Diamond I rank) and The Elder Scrolls Online (ESO:OL).
- 💬 Passionate about discussing C++, mathematics, abstraction, and all kinds of game-related topics.
- 🌌 Curious about the intersection of AI, neuroscience, and interactive digital experiences.

### Contact

- 📫 Email: buzzligh.frank@qq.com

---

## 你好 👋

### 关于我

🎓 软件工程专业研究生，研究方向为人工智能与神经科学的融合，主要探索脑机接口（BCI）技术与智能神经解码算法。

### 科研与技术兴趣

- 🧠 AI与神经科学的结合
- 🧩 脑机接口（BCI）技术
- 🧑‍💻 图形编程与游戏开发
- 💻 编程语言：C++、Python

### 代表项目

- 🛠️ [**ReduxGL**](https://github.com/VerdantE1/A-Modern-C-20-OpenGL-Rendering-Engine-with-Object-Oriented-Library)：  
  基于现代C++20与OpenGL的渲染引擎框架，采用面向对象设计，集成多项高级图形技术。  
  - 支持OpenGL Core Profile 3.3+  
  - 灵活的着色器与资源管理  
  - 动态场景渲染  
  - 展现现代图形编程的最佳实践

### 兴趣爱好 & 讨论话题

- 🎮 热衷于图形编程和游戏开发，从引擎架构到玩法系统与视觉表现都很感兴趣。
- 🕹️ 涉猎多种游戏类型，尤其喜欢策略类游戏（如P社四萌、上古卷轴、Bethesda系列）。
- 🌐 也活跃于网络游戏，例如英雄联盟（LOL，钻石1段位）、上古卷轴Online（ESO:OL）等。
- 💬 热衷于讨论 C++、数学、抽象、以及各类游戏相关话题。
- 🌌 对AI、神经科学与交互式数字体验的交叉领域充满好奇。

### 联系方式

- 📫 邮箱：buzzligh.frank@qq.com

---

感谢你的访问！欢迎交流科研、图形编程、C++、数学、抽象、游戏等话题！

=========================================
